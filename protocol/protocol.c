...
static void removeTrackSuffix(const char *input, char *output, size_t outputSize) {
    ...
    if (len >= 6) {
        ...
        if (len >= suffixLen &&
            strcmp(output + len - suffixLen, suffix) == 0) {
...
static int findPeerIndexUnlocked(const TrackerInfo *tracker,
                                 const char *ip,
                                 uint16_t port) {
...
static CommandOutput createTrackerHandler(const char *arg) {
    ...
    for (size_t i = 2; i < tokenCount - 3; i++) {
        int written = snprintf(description + descPos,
                               sizeof(description) - descPos,
                               "%s%s",
                               (i == 2 ? "" : " "),
                               tokens[i]);
...
static CommandOutput updateTrackerHandler(const char *arg) {
    ...
    if (!isNumberString(startStr) || !isNumberString(endStr) || !isNumberString(portStr)) {
...
static CommandOutput listHandler(const char *arg) {
    ...
    for (int i = 0; i < trackerCount; i++) {
        ...
        printf("%zu %s %zu %s [trackerId=%zu]\n",
               displayIndex,
               trackerArray_g[i].filename,
               trackerArray_g[i].filesize,
               trackerArray_g[i].md5Hash,
               trackerArray_g[i].trackerId);
        displayIndex++;
    }
...
static CommandOutput getHandler(const char *arg) {
    ...
    for (size_t i = 0; i < tracker->numPeers; i++) {
        printf("%s:%u:%zu:%zu:%ld\n",
               ...
               tracker->Peers[i].startByte,
               tracker->Peers[i].endByte,
...
CommandInfo parseCommand(char *line) {
    ...
    if (line == NULL) {
        printf("parseCommand: null input\n");
        return info;
    }
    ...
    if (cmd == CMD_GET || cmd == CMD_CREATE_TRACKER || cmd == CMD_UPDATE_TRACKER) {

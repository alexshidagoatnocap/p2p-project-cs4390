...
int main() {
  ...
  if ((socket(AF_INET, SOCK_STREAM, 0)) <
      0) { // create socket connection oriented
  ...
  // now associate the socket with local port to allow listening incoming
  // connections

  server_addr.sin_family = AF_INET; // assign address family

  server_addr.sin_port =
      htons(server_port); // change server port to NETWORK BYTE ORDER

  server_addr.sin_addr.s_addr = htons(INADDR_ANY);

  if (bind(sockid, (struct sockaddr *)&server_addr, sizeof(server_addr)) ==
      -1) { // bind and check error
  ...
  while (1) { // accept  connection from every requester client
    ...
    if ((sock_child =
             accept(sockid, (struct sockaddr *)&client_addr, &clilen)) ==
        -1) { /* accept connection and create a socket descriptor for actual
               * work
               */
    ...
    if ((pid = fork()) ==
        0) { // New child process will serve the requester client. separate
             // child will serve separate client
    ...
    close(sock_child); // parent all done with client, only child will
                       // communicate with that client from now

  } // accept loop ends

} // main fun ends

peer_handler(int sock_child) { // function for file transfer. child process will
                               // call this function
  ...
  if ((!strcmp(read_msg, "REQ LIST")) || (!strcmp(read_msg, "req list")) ||
    ...
    printf("list request handled.\\n");
           ...
           (strstr(read_msg, "GET") != NULL)) { // get command received

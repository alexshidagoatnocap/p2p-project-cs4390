...
int main(int argc, char *argv[]) {
  ...
  if (connect(sockid, (struct sockaddr *)&server_addr,
              sizeof(struct sockaddr)) == -1) { // connect and error check
  ...
  if (!strcmp(argv[1], "list")) { // if this is LIST command
    ...
    if ((write(sockid, (char *)&list_req, sizeof(list_req))) <
        0) { // inform the server of the list request

      printf("Send_request  failure\\n");
      exit(0);
    }

    if ((read(sockid, (char *)&msg, sizeof(msg))) <
        0) { // read what server has said
